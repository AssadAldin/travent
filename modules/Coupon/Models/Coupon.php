...
class Coupon extends BaseModel
{
    ...
    public function applyCouponValidate($booking, $action)
    {
        ...
        // dd($booking->coupons)
        $one_time_coupon = CouponBookings::where("booking_id", $booking->id)->first();
        if (!empty($one_time_coupon)) {
            return [
                'status' => 0,
                'message' => __("You can only use one coupon!")
            ];
        }
        ...
        // if ($booking->coupon->from ?? '') {
            if (!($booking->start_date >= $this->from && $booking->end_date <= $this->to)) {
                return [
                    'status' => 0,
                    'message' => __("You cant't apply this coupon in this day!(Coupon with Time Range) ")
                ];
            }
        // }
        return true;
    ...
    public function add($booking)
    {
        ...
        if ($this->without_service_fees) {
            $booking->total -= ($booking->total_before_fees / 100) * $this->service_amount;
        }
        ...
        $couponBooking->fill([
            ...
            'without_service_fees' => $this->without_service_fees,
            'service_amount' => $this->service_amount ?? 0,
