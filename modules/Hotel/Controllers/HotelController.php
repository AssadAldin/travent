...
class HotelController extends Controller
{
    ...
    {
        $newStart = implode('-', array_reverse(explode('/', $request->start)));
        $newEnd = implode('-', array_reverse(explode('/', $request->end)));
        $filters = [
            "start_date" => $newStart,
            "end_date" => $newEnd,
            "adults" => 1,
        ...
        foreach ($hotels as $hotel) {
            foreach ($hotel->rooms as $room) {
                if (!$room->isAvailableAtNoIcal($filters)) {
                    $i = $i + 1;
                    if (count($hotel->rooms) != $i) {
                        array_push($notAval, $hotel);
                        // break;
                    }
                }
            }
        ...
        foreach ($hotels as $hotel) {
            ...
            foreach ($hotel->rooms as $room) {
                if ($room->isAvailableAtNoIcal($filters)) {
                    $roomDates = $room->getDatesInRange($filters['start_date'], $filters['end_date']);
                    $dateUnq = $roomDates->sortByDesc('created_at')->unique('start_date');
                    $date1 = Carbon::parse(date('Y-m-d', strtotime($filters['start_date'])));
                    $date2 = Carbon::parse(date('Y-m-d', strtotime($filters['end_date'])));
                    $betweenDays = $date1->diffInDays($date2); // Count days between
                    $y = 0;
                    $ch = 0;
                    if (count($dateUnq)) {
                        if (count($dateUnq)) {
                            foreach ($dateUnq as $date) {
                                $tPriceDayStay += $date->price;
                                $tPriceFullDay += $date->price2;
                                if ($ch == 0) {
                                    $lessPriceDayStay = $date->price;
                                    $lessPriceFullDay = $date->price2;
                                    $ch = 1;
                                }
                                $y++;
                            }
                    ...
                    for ($y; $y <= $betweenDays; $y++) {
                        $tPriceDayStay += $room->price;
                        $tPriceFullDay += $room->price2;
                        if ($ch == 0) {
                            $lessPriceDayStay = $room->price;
                            $lessPriceFullDay = $room->price2;
                        }
                    }
                    // $hotel['dPrice'] = $tPriceDayStay;
                    // $hotel['fPrice'] = $tPriceFullDay;
                    $hotel['lessPrD'] = $lessPriceDayStay;
                    $hotel['lessPrF'] = $lessPriceFullDay;
                    $hotel['between'] = $betweenDays;
                    array_push($aval, $hotel);
                    break;
                }
            }
        }
    ...
    public function checkAvailability()
    {
        ...
        if (\request()->input('firstLoad') == "false") {
            ...
            $mixed_days = false;
            for ($i = 0; $i < 7; $i++) {
                if (str_contains($hotel->weekends_days, $i)) {
                    $count_weekend += 1;
            ...
            $weekday_array2 = implode(' ', $weekday_array);
            foreach ($period as $dt) {
                ...
                if (str_contains($hotel->weekends_days, $day_index)) {
                    if (!empty($hotel->full_weekend) and $numberDays != $count_weekend) {
                        return $this->sendError(__("You have to book all weekends days without any week days"));
                    }
                }
            }
            if (!empty($hotel->full_weekend) and $mixed_days) {
                return $this->sendError(__("You have to book all weekends days without any week days"));
            }
