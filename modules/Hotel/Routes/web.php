...
Route::get('/all/hotel/availability/user','AvailabilityController@all_availability_user')->name('hotel.admin.all.availability.user');
Route::redirect('/qr', '/app');
Route::redirect('/app', 'https://travent.onelink.me/7qvK/sle6h12a');
...
Route::get('/loadDates/all/vendor','AvailabilityController@loadDatesAll')->name('hotel.vendor.room.availability.loadDates.all');
...
Route::group(['prefix'=>'user/'.config('hotel.hotel_route_prefix')],function(){
    Route::group(['prefix'=>'{hotel_id}/availability'],function(){
        ...
        Route::get('/loadDatesGuest','AvailabilityController@loadDatesGuest')->name('hotel.vendor.room.availability.loadDatesGuest');
        Route::post('/store','AvailabilityController@store')->name('hotel.vendor.room.availability.store');
        Route::get('/show/availability','AvailabilityController@avail')->name('show.avail.hotel');
    });
});
