...
class Hotel extends Bookable
{
            ...
            foreach ($this->tmp_selected_rooms as $room) {
                if (isset($this->tmp_rooms_by_id[$room['id']])) {
                    if ($price_type == 1) {
                        ...
                        $total += $this->tmp_rooms_by_id[$room['id']]->tmp_price2 * $room['number_selected'];
                    }
                ...
                foreach ($this->tmp_selected_rooms as $room) {
                    ...
                    if (isset($this->tmp_rooms_by_id[$room['id']])) {
                        ...
                        $hotelRoomBooking->fillByAttr([
                            ...
                            'price2' => $this->tmp_rooms_by_id[$room['id']]->tmp_price2,
    ...
    public function addToCartValidate(Request $request)
    {
        ...
        $numberDays2 = abs(strtotime($end_date) - strtotime($start_date)) / 86400;
        ...
        foreach ($selected_rooms as $room) {
            ...
            $mixed_days = false;
            for ($i = 0; $i < 7; $i++) {
                if (str_contains($this->weekends_days, $i)) {
                    $count_weekend += 1;
                    array_push($weekend_array, $i);
                } else {
                    array_push($weekday_array, $i);
                }
            }
            $period = periodDate(request('start_date'), request('end_date'), true);
            foreach ($period as $dt) {
                if (strtotime(\request('end_date')) != strtotime($dt->format('Y-m-d H:i:s'))) {
                    $day_index = date("w", strtotime($dt->format('Y-m-d H:i:s')));
                }
                if (in_array($day_index, $weekday_array)) {
                    $have_weekday = true;
                }
                if (in_array($day_index, $weekend_array)) {
                    $have_weekend = true;
                }
                if ($have_weekday and $have_weekend) {
                    $mixed_days = true;
                }
            }
            // if (!empty($this->full_weekend) and $numberDays2 != $count_weekend) {
            //     return $this->sendError(__("You have to book all weekends days without any week days"));
            // }
            if (!empty($this->full_weekend) and $mixed_days) {
                return $this->sendError(__("You have to book all weekends days without any week days"));
            }
        }
        ...
        if ($this->enable_extra_price) {
            ...
            if (!empty($booking_data['extra_price'])) {
                foreach ($booking_data['extra_price'] as $k => &$type) {
                    ...
                    $type['price_html2'] = format_money($type['price']);
                    $type['price_type'] = '';
                    switch ($type['type']) {
                        case "per_day":
    ...
    public function getRoomsAvailability($filters = [])
    {
        ...
        foreach ($rooms as $room) {
            if ($firstItem) {
                $firstItem = false;
                $selected = 1;
                if ($room->full_day && $room->day_stay) {
                    $selected = 1;
                } else {
                    $selected = 0;
                }
            ...
            if ($room->isAvailableAt($filters)) {
                ...
                $res[] = [
                    ...
                    'price_html' => format_money($room->tmp_price) . '<span class="unit">/' . "1 night" . '</span>',
                    'price_html2' => format_money($room->tmp_price2) . '<span class="unit">/' . "1 night" . '</span>',
                    ...
                    'terms' => $terms,
                    'service_fee' => $room->tmp_price/100*8,
                    'service_fee2' => $room->tmp_price2/100*8,
                    'term_features' => $term_features,
                    'fullDay' => $room->full_day,
                    'dayStay' => $room->day_stay,
                    'weekendsPrice' => $room->weekendsPrice,
                    'weekendsPrice2' => $room->weekendsPrice2,
                ];
                $this->tmp_rooms[] = $room;
            }
        }
    ...
    public function detailBookingEachDate($booking)
    {
        ...
        foreach ($rooms as $r => $room) {
            ...
            foreach ($period as $dt) {
                ...
                $price2 = $room->room->price2 * $room->number;
                $date['price'] = $price;
                $date['price_html'] = format_money($price);
                $date['price_html2'] = format_money($price2);
        ...
        if (!empty($roomsDates)) {
            foreach ($roomsDates as $roomsDate) {
                ...
                if ($allDates[$roomsDate->target_id][date('Y-m-d', $startDate)]) {
                    ...
                    $price2 = $roomsDate->price2 * $roomsNumber[$roomsDate->target_id] ?? 1;
                    $date['price'] = $price;
                    $date['price_html'] = format_money($price);
                    $date['price_html2'] = format_money($price2);
                    $date['from'] = $startDate;
    ...
    {
        $model_hotel = parent::query()->with(['rooms'])->select("bravo_hotels.*");
        //        $model_hotel = parent::query()->whereBetween('created_at', ['2023-10-08 09:30:29', '2023-11-20 14:20:14']);
    ...
    // Cancelation Policy relationship
    public function policy_n()
    {
        return $this->belongsTo(HotelCancelationPolicy::class, "cancelation_id");
    }
