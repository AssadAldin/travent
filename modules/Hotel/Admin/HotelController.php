...
class HotelController extends AdminController
{
    ...
    public function all_availability(Request $request)
    {
        ...
        return view('Hotel::admin.all_availability', compact('rows', 'breadcrumbs', 'current_month', 'page_title', 'request'));
    }
    ...
    public function store(Request $request, $id)
    {
        ...
        $dataKeys = [
            ...
            'service_fee',
            'surrounding',
            'cancelation_id', // Add cancelation to hotel table
            'full_weekend',
            'weekends_days',
            'discount_percent1',
            'discount_code',
        ...
        if ($request->input('cancelation_id') == null) {
            $row->cancelation_id = 1;
        }
        ...
        if ($request->input('full_weekend') == null) {
            $row->full_weekend = 0;
        }
        if ($id >= 0) {
            $row->weekends_days =
    ...
    public function sortImage(Hotel $hotel)
    {
        $images = $hotel->gallery;
        $imagesIds = explode(',', $images);
        $files = array();
        foreach ($imagesIds as $id) {
            //            if((count($imagesIds) - 1) != $i){
            array_push($files, MediaFile::find($id));
            //            }
        }
        //        dd($files);
        return view('Hotel::frontend.sortImage', [
            'hotel' => $hotel,
            'images' => $images,
            'imagesIds' => $imagesIds,
            'files' => $files,
    ...
    // Sort image edited by Assadaldin
    public function saveSort(Request $request, $id)
    {
        ...
        // update priority
        $file->priority = $request->input('priority');
        $file->save();
