...
class PayoutController extends FrontendController
{
    ...
    public function storeAdminPayoutAccounts(Request $request, $id)
    {
        $request->validate([
            'bank_name' => 'required',
            'bank_IBAN' => 'required',
        ...
        $user = User::find($id);
        $user->addMeta('vendor_payout_accounts', request()->input('payout_accounts'), request()->input('bank_user_name'), request()->input('bank_name'), request()->input('bank_IBAN'));
        $userMeta = UserMeta::where("user_id", $user->id)->where("name", "vendor_payout_accounts")->first();
        $userMeta->bank_name = $request->input('bank_name');
        $userMeta->bank_IBAN = $request->input('bank_IBAN');
        $userMeta->bank_user_name = $request->input('bank_user_name');
        $userMeta->save();
