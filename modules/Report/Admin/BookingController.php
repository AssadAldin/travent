...
class BookingController extends AdminController
{
    ...
    public function index2(Request $request)
    {
        ...
        if ($request->from and $request->to) {
            ...
            $data = Booking::where('status', '!=', 'draft')->orderBy('created_at', 'asc')->get();
        }
        return view('Report::admin.booking.index2', compact('data'));
    }
    ...
    public function booking_not_completed(Request $request)
    {
        $this->checkPermission('booking_view');
        $query = Booking::where('status', '=', 'draft');
        if (!empty ($request->s)) {
        ...
        return view('Report::admin.booking.index_not', $data);
    }
