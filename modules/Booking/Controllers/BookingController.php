...
class BookingController extends \App\Http\Controllers\Controller
{
    ...
    public function doCheckout(Request $request)
    {
                ...
                if (count($booking->coupons)) {
                    foreach ($booking->coupons as $coupon) {
                        if ($coupon->without_service_fees) {
                            $booking->total -= ($booking->total_before_fees / 100) * $coupon->service_amount;
                        } else {
        ...
        $booking->cancelation = $booking->service->policy_n->item ?? 4; // Add cancelation policy in booking
        ...
        if ($booking->pay_now > 0) {
            ...
            // event(new SendWhatsappEvent($booking)); // In prodiction whatsapp message must be here
