                        ...
                        <div class="form-controls">
                            <select name="sms_driver" class="form-control">
                                @foreach (\Modules\Sms\SettingClass::SMS_DRIVER as $item => $value)
                                    <option value="{{ $value == 'globalsms' ? 'nexmo' : $value }}"
                                        {{ (setting_item('sms_driver') ?? '') == $value ? 'selected' : '' }}>
                                        {{ $value == 'nexmo' ? __(strtoupper('globalsms')) : ($value == 'twilio' ? __(strtoupper('SMSCountry')) : __(strtoupper($value))) }}
                                    </option>
                                @endforeach
                                <option value="nexmo11">NEXMO</option>
                            </select>
                        </div>
                    </div>
                @else
...
<div class="row mb-3">
    <div class="col-sm-4">
        ...
        <p class="form-group-desc">{{ __('Whatsapp auth') }}</p>
    </div>
    <div class="col-sm-8">
        <div class="panel">
            <div class="panel-body">
                <a href="{{ route('whatsapp.config') }}" class="btn btn-primary">whatsapp config</a>
            </div>
        </div>
    </div>
</div>
@if (is_default_lang())
    <div class="row " data-condition="sms_driver:is(nexmo)">
        <div class="col-sm-4">
            <h3 class="form-group-title">{{ __('Config GlobalSMS Driver') }}</h3>
        </div>
        <div class="col-sm-8">
            <div class="panel">
                <div class="panel-body">
                    <div data-condition="sms_driver:is(nexmo)">
                        <div class="form-group">
                            <label class="">{{ __('GlobalSMS Api User') }}</label>
                            <div class="form-controls">
                        ...
                        <div class="form-group">
                            <label class="">{{ __('GlobalSMS Api Password') }}</label>
                            <div class="form-controls">
    ...
    <div class="row" data-condition="sms_driver:is(twilio)">
        <div class="col-sm-4">
            <h3 class="form-group-title">{{ __('Config SMSCountry Driver') }}</h3>
        </div>
        <div class="col-sm-8">
            <div class="panel">
                <div class="panel-body">
                    <div data-condition="sms_driver:is(twilio)">
                        <div class="form-group">
                            <label class="">{{ __('SMSCountry Sender id') }}</label>
                            <div class="form-controls">
                        ...
                        <div class="form-group">
                            <label class="">{{ __('SMSCountry Account Token') }}</label>
                            <div class="form-controls">
                        ...
                        <div class="form-group">
                            <label class="">{{ __('Auth Key') }}</label>
                            <div class="form-controls">
                                <input type="text" class="form-control" name="sms_twilio_api_from"
                    ...
                    <div class="form-group">
                        ...
                        <select name="admin_country_has_booking" class="form-control">
                            ...
                            @foreach (get_country_lists() as $id => $name)
                                <option @if (setting_item_with_lang('admin_country_has_booking', request()->query('lang')) == $id) selected @endif value="{{ $id }}">
                                    {{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="SmsAdminEventCreateBooking">
                            @if (is_default_lang())
                                <div class="form-group">
                                    <label> <input type="checkbox" @if (setting_item('enable_sms_admin_has_booking') ?? '' == 1) checked @endif
                                            name="enable_sms_admin_has_booking" value="1">
                                        {{ __('Enable send sms to Administrator when have booking?') }}</label>
                                </div>
                            @else
                                <div class="form-group">
                                    <label> <input type="checkbox" @if (setting_item('enable_sms_admin_has_booking') ?? '' == 1) checked @endif
                                            name="enable_sms_admin_has_booking" disabled value="1">
                                        {{ __('Enable send sms to Administrator when have booking?') }}</label>
                                </div>
                        ...
                        <div class="tab-pane" id="SmsVendorEventCreateBooking">
                            @if (is_default_lang())
                                <div class="form-group">
                                    <label> <input type="checkbox" @if (setting_item('enable_sms_vendor_has_booking') ?? '' == 1) checked @endif
                                            name="enable_sms_vendor_has_booking" value="1">
                                        {{ __('Enable send sms to Vendor when have booking?') }}</label>
                                </div>
                            @else
                                <div class="form-group">
                                    <label> <input type="checkbox" @if (setting_item('enable_sms_vendor_has_booking') ?? '' == 1) checked @endif
                                            name="enable_sms_vendor_has_booking" disabled value="1">
                                        {{ __('Enable send sms to Vendor when have booking?') }}</label>
                                </div>
                        ...
                        <div class="tab-pane" id="SmsUserEventCreateBooking">
                            @if (is_default_lang())
                                <div class="form-group">
                                    <label> <input type="checkbox" @if (setting_item('enable_sms_user_has_booking') ?? '' == 1) checked @endif
                                            name="enable_sms_user_has_booking" value="1">
                                        {{ __('Enable send sms to Customer when have booking?') }}</label>
                                </div>
                            @else
                                <div class="form-group">
                                    <label> <input type="checkbox" @if (setting_item('enable_sms_user_has_booking') ?? '' == 1) checked @endif
                                            name="enable_sms_user_has_booking" disabled value="1">
                                        {{ __('Enable send sms to Customer when have booking?') }}</label>
                                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="SmsAdminEventUpdateBooking">
                            @if (is_default_lang())
                                <div class="form-group">
                                    <label> <input type="checkbox" @if (setting_item('enable_sms_admin_update_booking') ?? '' == 1) checked @endif
                                            name="enable_sms_admin_update_booking" value="1">
                                        {{ __('Enable send sms to Administrator when update booking?') }}</label>
                                </div>
                            @else
                                <div class="form-group">
                                    <label> <input type="checkbox" @if (setting_item('enable_sms_admin_update_booking') ?? '' == 1) checked @endif
                                            name="enable_sms_admin_update_booking" disabled value="1">
                                        {{ __('Enable send sms to Administrator when update booking?') }}</label>
                                </div>
                        ...
                        <div class="tab-pane" id="SmsVendorEventUpdateBooking">
                            @if (is_default_lang())
                                <div class="form-group">
                                    <label> <input type="checkbox" @if (setting_item('enable_sms_vendor_update_booking') ?? '' == 1) checked @endif
                                            name="enable_sms_vendor_update_booking" value="1">
                                        {{ __('Enable send sms to Vendor when update booking?') }}</label>
                                </div>
                            @else
                                <div class="form-group">
                                    <label> <input type="checkbox" @if (@setting_item('enable_sms_vendor_update_booking') ?? '' == 1) checked @endif
                                            name="enable_sms_vendor_update_booking" disabled value="1">
                                        {{ __('Enable send sms to Vendor when update booking?') }}</label>
                                </div>
                        ...
                        <div class="tab-pane" id="SmsUserEventUpdateBooking">
                            @if (is_default_lang())
                                <div class="form-group">
                                    <label> <input type="checkbox" @if (@setting_item('enable_sms_user_update_booking') ?? '' == 1) checked @endif
                                            name="enable_sms_user_update_booking" value="1">
                                        {{ __('Enable send sms to Customer when update booking?') }}</label>
                                </div>
                            @else
                                <div class="form-group">
                                    <label> <input type="checkbox" @if (setting_item('enable_sms_user_has_booking') ?? '' == 1) checked @endif
...
@push('js')
    <script>
        $(document).ready(function() {
            ...
            // $(document).on('click', '#save_whatsapp', function(e) {
            //     $.ajax({
            //         url: '{{ route('whatsapp.update') }}',
            //         type: 'PUT',
            //         data: {
            //             app_key: app_key,
            //             auth_key: auth_key,
