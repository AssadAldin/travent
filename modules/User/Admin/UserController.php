...
class UserController extends AdminController
{
    ...
    public function createPayout($id){
        $row = User::find($id);
        return view('User::admin.createPayout', compact('row'));
    }
    public function storePayout(Request $request, $id){
        ...
        if(!is_array($vendor_payout_methods) or empty($vendor_payout_methods)){
            return back()->with('success', __("Sorry! No method available at the moment"));
        }
        $user = User::find($id);
        // dd($user);
        ...
        if(empty($user_available_methods) or empty($user_available_methods[$payout_method])){
            return back()->with('message', __("You does not select payout method or you need to enter account info for that method"));
        }

        if($user->available_payout_amount < $amount){
            return back()->with('message', __("You don not have enough :amount for payout",['amount'=>format_money($amount)]));
        }

        $method_detail = $user_available_methods[$payout_method];

        if(!empty($method_detail->min) and $method_detail->min > $amount){
            return back()->with('message', __("Minimum amount to pay is :amount",["amount"=>format_money($method_detail->min)]));
        }
        ...
        return back()->with('message', __('Payout request has been created'));
