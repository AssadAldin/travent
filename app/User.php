...
//        }
    ...
    public function userMoney($row)
    {
        $res = [];
        $total_price = 0;
        $total_earning = 0;
        $vendorPayout = 0;
        // sum( CASE WHEN `status` = "completed" THEN `total_before_fees` - `commission` + `vendor_service_fee_amount` ELSE NULL END ) AS total_earning'
        $bookings = Booking::where('vendor_id', $row->id)->whereNotIn('status', static::$notAcceptedStatus)->get();
        $payouts = VendorPayout::where('vendor_id', $row->id)->where('status', '!=', 'Rejected')->get();
        foreach($payouts as $payReq){
            $vendorPayout += $payReq->amount;
        }
        foreach ($bookings as $booking) {
            $total_price += $booking->total_before_fees - $booking->commission + $booking->vendor_service_fee_amount;
            if ($booking->status == 'completed') {
                $total_earning += $booking->total_before_fees - $booking->commission + $booking->vendor_service_fee_amount;
            }
        }
        $res = [
    ...
    {
        $url = Url::temporarySignedRoute('user.verify', now()->addMinutes(30), ['user' => $this]);
        // $actionUrl = $this->verificationUrl();
        $a = Mail::to($this->email)->send(new EmailUserVerifyRegister($this, $url));
    }
