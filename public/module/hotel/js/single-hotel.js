(function ($) {
    ...
    var hotelRoomForm = new Vue({
        ...
        data: {
            ...
            total_price_fee: 0,
            total_price_day_stay: 0,
            dayStayDisable: false,
            togglePrice: false,
            price_type: false,
        ...
        computed: {
            total_price: function () {
                ...
                if (this.firstLoad) {
                    if (this.qur.searchParams.get("startCheck") || this.qur.searchParams.get("endCheck")) {
                        this.start_date_html =
                            this.qur.searchParams
                                .get("startCheck")
                                .split("-").reverse().join("-")
                                .replaceAll("-", "/") +
                            ...
                            this.qur.searchParams
                                ...
                                .split("-").reverse().join("-")
                                .replaceAll("-", "/");
                        this.start_date = this.qur.searchParams.get("startCheck");
                        this.end_date = this.qur.searchParams.get("endCheck");
                        this.checkAvailability();
                    }
                }
                var me = this;
                this.dayStayDisable = moment(this.end_date).diff(moment(this.start_date), 'day') !== 1;
                // console.log(moment(this.end_date).diff(moment(this.start_date), 'day'));
                if (me.start_date !== "" && me.total_rooms > 0) {
                    var guests = me.children + me.adults;
                    var total_price = 0;
                    var startDate = new Date(me.start_date).getTime();
                    var endDate = new Date(me.end_date).getTime();
                    // var parts =me.start_date.split('-');
                    ...
                    if (this.togglePrice) {
                        ...
                        _.forEach(this.rooms, function (item) {
                            ...
                            if (item.number_selected) {
                                ...
                                //     total_price += item.weekendsPrice2 * parseInt(item.number_selected);
                                // }else{
                                // }
                                total_price += item.price2 * parseInt(item.number_selected);
                            }
                        });
                    }
                    ...
                    if (this.total_price_day_stay) {
                        this.checkAvailability();
                        this.total_price_day_stay = 0;
                    }
                    if (this.totPriceStart == 0) {
                        total_price = 0
                    }
                    return total_price;
                }
            ...
            total_price_html2: function () {
                if (!this.total_price) return '';
                setTimeout(function () {
                    $('[data-toggle="tooltip"]').tooltip();
                    $(document).trigger("scroll");
                }, 200);
                // console.log(this.total_price_before_fee);
        ...
        methods: {
            ...
            fetchEvents(start, end) {
                ...
                $.ajax({
                    url: bravo_booking_i18n.load_dates_url1,
                    dataType: "json",
                    type: 'get',
                    data: data,
                    beforeSend: function () {
            ...
            checkAvailability: function () {
                ...
                $.ajax({
                    ...
                    success: function (json) {
                        ...
                        me.firstLoad = false;
                        // console.log(me.onLoadAvailability);
                        if (json.rooms) {
                            me.rooms = json.rooms;
            ...
            doSubmit: function (e, coupon) {
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        if (res.url) {
                            window.location.href = res.url + "?" + "coupon=" + coupon
                        }
            ...
            openStartDate: function () {
                if (!this.dayStayDisable) {
                    // this.checkAvailability();
                    // location.reload();
                    this.total_price_day_stay = 1;
                    if (!this.firstLoad) {
                        this.togglePrice = false;
                        this.price_type = 0;
                        // location.reload();
                ...
                if (!this.firstLoad && this.total_price > 0) {
                    this.totPriceStart = 0;
                }
