    ...
    <div class="row">
        ...
        <div class="col-md-9">
            <div class="item-title">
                <a href="{{route('hotel.vendor.room.edit',['hotel_id'=>$hotel->id,'id'=>$row->id])}}?lang=en">
                    {{$translation->title}}
                </a>
            </div>
            <div class="location">
            ...
            <div class="control-action">
                @if(Auth::user()->hasPermission('hotel_update'))
                    <a href="{{route('hotel.vendor.room.edit',['hotel_id'=>$hotel->id,'id'=>$row->id])}}?lang=en" class="btn btn-warning">{{__("Edit")}}</a>
                @endif
                @if(Auth::user()->hasPermission('hotel_update'))
