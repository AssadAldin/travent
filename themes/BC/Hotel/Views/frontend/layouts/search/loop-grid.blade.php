    ...
    <a @if (!empty($blank)) target="_blank" @endif href="{{ $row->getDetailUrl() }}">
        <div class="item-title text-dark">
            ...
            @if ($row->discount_percent1)
                <div class="sale_info font-weight-bold text-center" style="font-size: x-small;height: 70px; width: 70px; background-color:red;">{{ $row->discount_percent1 }}</div>
            @endif
        </div>
        <div class="location">
        ...
        @if ($row->discount_code)
            <div class="info">
                <div class="g-price">
                    ...
                    <div class="price">
                        <span class="text-price" style="color: red;"> {{ $row->discount_code }}</span>
                    </div>
                </div>
            </div>
        @endif
