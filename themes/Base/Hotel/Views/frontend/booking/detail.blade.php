            ...
            <ul class="review-list">
                @if ($booking->start_date)
                    ...
                    @if ($booking->price_type == 1)
                        ...
                        <li>
                            <div class="label">{{ __('Day Stay') }}</div>
                            <div class="val">
                    ...
                    @else
                        ...
                        <li>
                            <div class="label">{{ __('Full Day') }}</div>
                            <div class="val">
                                {{ __('Check in time') }}&nbsp; <b>{{ $service->check_in_time }}</b> <br>
                                {{ __('Check out time') }}&nbsp; <b>{{ $service->check_out_time }}</b>
                            </div>
                        </li>
                    @endif
                @endif
                            ...
                            if (count($booking->coupons)) {
                                foreach ($booking->coupons as $coupon) {
                                    if ($coupon->without_service_fees) {
                                        ...
                                        $booking->total = $booking->total_before_fees + $coupon->coupon_amount;
                                        if ($coupon->coupon_amount) {
                                            $booking->total -= $coupon->coupon_amount;
                                        }
                                    } else {
                            ...
                            <li class="">
                                ...
                                <div class="val">
                                    ...
                                    @else
                                        ...
                                        @else
                                            <s class="text-danger">{{ format_money($fee_price) }}</s>
                                        @endif
                                    @endif
                                </div>
                            </li>
                ...
                {{-- @php
                    ...
                    if (count($booking->coupons)) {
                        foreach ($booking->coupons as $coupon) {
                            if ($coupon->without_service_fees) {
                                // dd($coupon);
                                $fees_service = $coupon->coupon_amount - $fee_price;
                            }
                        }
                    }
                @endphp --}}
                        ...
                        @foreach ($rooms as $room)
                            <li class="mb-3 pb-1 border-bottom">
                                ...
                                @if (!empty($dateDetail[$room->room_id]))
                                    <div>
                                        @includeIf('Hotel::frontend.booking.detail-room', [
                                            'roomDate' => $dateDetail[$room->room_id],
                                        ])
                                    </div>
                                @endif
