    ...
    <form action="{{ route('user.profile.update') }}" method="post" class="input-has-icon">
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="form-group">
                    ...
                    <input type="text" required minlength="4" name="user_name"
                        value="{{ old('user_name', $dataUser->user_name) }}" placeholder="{{ __('User name') }}"
                        class="form-control">
                    <i class="fa fa-user input-icon"></i>
                </div>
                <div class="form-group">
                ...
                @if ($is_vendor_access)
                    <div class="form-group">
                        <label>{{ __('Phone For Guest') }} <small class="font-weight-bold">
                                {{ __('(For Booking E-mail)') }}</small></label>
                        <input type="text"
                            value="@if ($dataUser->phone2) {{ $dataUser->phone2 }}@else{{ $dataUser->phone }} @endif"
                            name="phone2" placeholder="{{ __('Phone Number') }}2" class="form-control">
                        <i class="fa fa-phone input-icon"></i>
                    </div>
                    <div class="form-group">
                        <label>{{ __('Phone For whatsapp') }} <small class="font-weight-bold">
                                {{ __('(For Booking whatsapp)') }}</small></label>
                        <input type="text"
                            value="@if ($dataUser->phone3) {{ $dataUser->phone3 }}@else{{ $dataUser->phone }} @endif"
                            name="phone3" placeholder="{{ __('Phone Number') }}2" class="form-control">
                        <i class="fa fa-phone input-icon"></i>
                    </div>
                @endif
                <div class="form-group">
            ...
            <div class="col-md-6">
                ...
                <div class="form-group">
                    ...
                    <select name="country" class="form-control">
                        ...
                        @foreach (get_country_lists() as $id => $name)
                            <option @if (old('country', $dataUser->country ?? '') == $id) selected @endif value="{{ $id }}">
                                {{ $name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    <label>{{ __('Notification Language') }}</label>
                    <select style="height:40px;" name="user_notifi_lang" class="form-control">
                        <option value="">{{ __('-- Select --') }}</option>
                        @foreach ($languages as $language)
                            <option @if ($language->locale == $dataUser->user_notifi_lang) selected @endif value="{{ $language->locale }}">{{ $language->name }}</option>
                        @endforeach
    ...
    @if (!empty(setting_item('user_enable_permanently_delete')) and !is_admin())
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <a data-toggle="modal" data-target="#permanentlyDeleteAccount" class="btn btn-danger"
                    href="">{{ __('Delete your account') }}</a>
            </div>

            <!-- Modal -->
            <div class="modal  fade" id="permanentlyDeleteAccount" tabindex="-1" role="dialog"
